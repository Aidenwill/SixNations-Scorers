...
if (Test-Path $highScoresFilePath) {
    try {
        $loadedScores = Get-Content $highScoresFilePath -Raw | ConvertFrom-Json
        # Ensure it's an array
        if ($loadedScores -is [array]) {
            $highScores = [System.Collections.ArrayList]$loadedScores
        } elseif ($loadedScores) {
            $highScores = [System.Collections.ArrayList]@($loadedScores)
        } else {
            $highScores = [System.Collections.ArrayList]@()
        }
    } catch {
...
function Play-Round {
    ...
    if ($isCorrect) {
        ...
        if ($streak.Value -gt $script:bestStreak) {
            $script:bestStreak = $streak.Value
        }
...
function Save-HighScore {
    ...
    # Use global variable
    $script:highScores.Add($newEntry) | Out-Null
...
function Show-HighScores {
    ...
    if ($script:highScores.Count -eq 0) {
        Write-Host "No high scores recorded yet."
    } else {
        $script:highScores | Sort-Object -Property score -Descending | Select-Object -First 10 | ForEach-Object {
...
$bestStreak = 0  # Track the best streak achieved in this session
...
# Save high score if achieved (use best streak from session)
if ($script:bestStreak -gt 0) {
    Save-HighScore $playerName $script:bestStreak
}
...
Write-Host "Best streak this session: $script:bestStreak"
